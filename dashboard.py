...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} html,body{margin:0;min-height:100%;background:radial-gradient(circle at 20% -10%,rgba(113,112,255,.22),transparent 34%),radial-gradient(circle at 90% 0%,rgba(34,211,238,.12),transparent 30%),var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,sans-serif;font-feature-settings:"cv01","ss03"} body{padding:28px}.wrap{max-width:1440px;margin:0 auto}
    ...
    .timeline{display:flex;gap:8px;align-items:end;height:130px;padding-top:12px}.bar{flex:1;min-width:8px;border-radius:6px 6px 2px 2px;background:linear-gradient(180deg,var(--accent2),rgba(113,112,255,.28));position:relative}.bar.err{background:linear-gradient(180deg,var(--bad),rgba(239,68,68,.25))}.bar:hover:before{content:attr(data-tip);position:absolute;bottom:calc(100% + 8px);left:50%;transform:translateX(-50%);background:#191a1b;border:1px solid var(--line);border-radius:8px;padding:7px 9px;color:var(--text);font:12px ui-monospace;white-space:pre;z-index:5}.kv{display:grid;grid-template-columns:150px 1fr;gap:8px;font-size:13px}.kv div:nth-child(odd){color:var(--dim)}.kv div:nth-child(even){color:var(--soft);overflow-wrap:anywhere}.empty{padding:60px;text-align:center;color:var(--muted)}footer{color:var(--dim);font-size:12px;margin:18px 0 4px;text-align:center}.tabs{display:flex;gap:8px;margin-bottom:12px}.tab{cursor:pointer;border:1px solid var(--line);background:rgba(255,255,255,.03);color:var(--muted);padding:7px 10px;border-radius:9px;font-size:12px}.tab.active{color:var(--text);background:rgba(113,112,255,.18);border-color:rgba(113,112,255,.45)}
...
function xLabelFor(row){return row?.date||row?.hour||shortDateLabel(row?.t)||row?.period_label||(row?.interval||row?.timeInterval||'').split('/')[0].slice(0,10)}
function finiteOrNull(raw){if(raw==null||raw==='')return null;let v=+raw;return Number.isFinite(v)?v:null}
function drawXAxisLabels(ctx,items,xFor,y,opt={}){let n=items.length;if(!n)return;let maxTicks=opt.maxTicks||6,step=Math.max(1,Math.ceil(n/maxTicks));ctx.save();ctx.fillStyle='rgba(208,214,224,.72)';ctx.strokeStyle='rgba(255,255,255,.10)';ctx.font='11px ui-monospace';ctx.textAlign='center';ctx.textBaseline='top';for(let i=0;i<n;i+=step){let label=xLabelFor(items[i]);if(!label)continue;let x=xFor(i);ctx.beginPath();ctx.moveTo(x,y-8);ctx.lineTo(x,y-3);ctx.stroke();ctx.fillText(label,x,y)}if(n>1&&(n-1)%step!==0){let label=xLabelFor(items[n-1]);let x=xFor(n-1);ctx.beginPath();ctx.moveTo(x,y-8);ctx.lineTo(x,y-3);ctx.stroke();ctx.fillText(label,x,y)}ctx.restore()}
...
<section class="card pad side"><div class="section-title"><h2>Coverage and run health</h2><span>poll history</span></div><div class="timeline">${DASHBOARD_DATA.runs.map(r=>`<div class="bar ${r.error?'err':''}" style="height:${Math.max(8,Math.min(120,(r.usage_rows||1)/(latest.usage_rows||1)*120))}px" data-tip="${esc(r.retrieved_label)}\n${r.usage_rows} usage rows · ${r.cost_rows} cost rows\n${fmt(r.total_net_kwh,3)} kWh · ${money(r.total_cost)}\n${r.error?'ERROR: '+JSON.stringify(r.error):'ok'}"></div>`).join('')}</div><div class="kv" style="margin-top:18px"><div>Last successful poll</div><div>${esc(latest.retrieved_label)}</div><div>AMI available through</div><div class="mono">${esc(latest.latest_available_end||'unknown')}</div><div>Data directory</div><div class="mono">${esc(latest.run_dir)}</div><div>Available interval</div><div class="mono">${esc(latest.available_interval)}</div><div>Requested interval</div><div class="mono">${esc(latest.requested_interval)}</div></div></section>
...
<section class="card pad full"><div class="section-title"><h2>Canonical kWh price over time</h2><span>${DASHBOARD_DATA.historical_price_series.length.toLocaleString()} historical intervals across downloaded runs</span></div><canvas id="priceChart"></canvas></section>
...
function drawQuarterHour(){lineChart($('usageChart'),latest.series,selectedDefs(),{title:'Quarter-hour reading',rightAxis:selectedDefs().some(d=>d.axis==='right')})}
function drawAll(){drawQuarterHour();barChart($('dailyChart'),latest.daily,'net_kwh','#7170ff',{title:'Daily usage'});barChart($('costChart'),latest.daily,'cost','#34d399',{title:'Daily cost'});lineChart($('priceChart'),DASHBOARD_DATA.historical_price_series.filter(x=>x.price_per_kwh!=null),[{key:'price_per_kwh',label:'Historical kWh price',color:'#f472b6',format:rate}],{title:'Historical interval price',yDigits:3,zero:false});lineChart($('billTrendChart'),DASHBOARD_DATA.bill_trend||[],[{key:'kwh',label:'Bill kWh',color:'#7170ff',format:v=>fmt(v,1)+' kWh'},{key:'usageCharges',label:'Usage charges',color:'#34d399',format:money},{key:'effective_price_per_kwh',label:'Actual $/kWh',color:'#f472b6',format:rate,axis:'right',dash:[5,5]}],{title:'Bill trend',rightAxis:true,yDigits:1,rightDigits:3,zero:false});lineChart($('demandEventChart'),(DASHBOARD_DATA.demand_event_series||[]).slice(0,30).slice().reverse(),[{key:'kw',label:'Demand',color:'#22d3ee',format:v=>fmt(v,3)+' kW'},{key:'mean_temperature',label:'Mean temp',color:'#f472b6',format:v=>fmt(v,1)+'°F',axis:'right',dash:[5,5]}],{title:'Peak demand events',rightAxis:true,yDigits:3,rightDigits:1})}
renderQhControls();drawAll();window.onresize=drawAll;$('footer').textContent=`Static file: refresh after each scraper run · Source data: ${DASHBOARD_DATA.data_dir}`}
render();
</script></body></html>'''
