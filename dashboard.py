...
def _duration_label(minutes: int | None) -> str:
    ...
    whole, rem = divmod(minutes, 60)
    return f"{whole}h {rem}m" if rem else f"{whole}h"


def _interval_display_label(interval: str) -> str:
    start = _parse_interval_datetime(_start_time(interval))
    if not start:
        return interval or "unknown"
    return start.strftime("%b %d, %-I:%M %p") if os.name != "nt" else start.strftime("%b %d, %#I:%M %p")
...
def summarize_run(run_dir: Path) -> dict[str, Any]:
    ...
    for r in net_usage:
        ...
        estimated_kwh_at_peak = demand_by_interval.get(interval, 0.0) * hours if minutes is not None else None
        estimated_cost_at_peak = estimated_kwh_at_peak * canonical_price if estimated_kwh_at_peak is not None and canonical_price is not None else None
        series.append({
            "t": _start_time(interval),
            "interval": interval,
            "label": _interval_display_label(interval),
            "duration_minutes": minutes,
            "duration_label": _duration_label(minutes),
            "estimated_kwh_at_peak": estimated_kwh_at_peak,
            "estimated_cost_at_peak": estimated_cost_at_peak,
            "kwh": kwh,
            "cost": cost,
    ...
    for r in source_peak_rows:
        ...
        interval = r.get("timeInterval") or ""
        minutes = _interval_minutes(interval)
        hours = (minutes or 0) / 60.0
        price_info = pricing_by_interval.get(interval, {})
        price = price_info.get("cost_per_unit")
        estimated_kwh = kw * hours if kw is not None and minutes is not None else None
        estimated_cost = estimated_kwh * price if estimated_kwh is not None and price is not None else None
        if kw is not None:
            peak_demand_events.append({
                ...
                "duration_minutes": minutes,
                "duration_label": _duration_label(minutes),
                "estimated_kwh_at_peak": estimated_kwh,
                "estimated_cost_at_peak": estimated_cost,
                "price_per_kwh": price,
                "kw": kw,
                "unit": r.get("unit"),
                "readType": r.get("readType"),
...
<section class="card pad half mini"><div class="section-title"><h2>Daily usage + weather</h2><span>tooltip includes temp</span></div><canvas id="dailyChart"></canvas></section><section class="card pad half mini"><div class="section-title"><h2>Cost by period</h2><span id="costGranularityLabel">daily cost</span></div><div class="filters" id="costGranularity"><label class="check"><input type="radio" name="costGranularity" data-cost-granularity="daily" checked>Daily</label><label class="check"><input type="radio" name="costGranularity" data-cost-granularity="hourly">Hourly</label></div><canvas id="costChart"></canvas></section>
...
let costGranularity='daily';
function dailyCostRows(){return latest.daily||[]}
function hourlyCostRows(){return latest.hourly||[]}
