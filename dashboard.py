...
<section class="card pad full"><div class="section-title"><h2>Canonical kWh price over time</h2><span>${DASHBOARD_DATA.historical_price_series.length.toLocaleString()} intervals · EIA regional/national monthly benchmarks when available</span></div><canvas id="priceChart"></canvas><div class="legend"><span style="--c:#f472b6">Eversource canonical</span><span style="--c:#22d3ee">EIA MA residential avg</span><span style="--c:#34d399">EIA US residential avg</span></div></section>
...
function drawAll(){drawQuarterHour();barChart($('dailyChart'),latest.daily,'net_kwh','#7170ff',{title:'Daily usage',timeLabels:'date'});barChart($('costChart'),latest.daily,'cost','#34d399',{title:'Daily cost',timeLabels:'date'});const eia=(DASHBOARD_DATA.price_benchmarks||[]).map(x=>({...x,t:(x.period||'')+'-01',regional_price:x.scope==='regional'?x.price_per_kwh:null,national_price:x.scope==='national'?x.price_per_kwh:null}));const priceRows=DASHBOARD_DATA.historical_price_series.filter(x=>x.price_per_kwh!=null).concat(eia);lineChart($('priceChart'),priceRows,[{key:'price_per_kwh',label:'Eversource canonical',color:'#f472b6',format:rate},{key:'regional_price',label:'EIA regional avg',color:'#22d3ee',format:rate,dash:[5,5]},{key:'national_price',label:'EIA national avg',color:'#34d399',format:rate,dash:[2,4]}],{title:'Canonical and benchmark price',yDigits:3,zero:false,timeLabels:'date'});lineChart($('billTrendChart'),DASHBOARD_DATA.bill_trend||[],[{key:'kwh',label:'Bill kWh',color:'#7170ff',format:v=>fmt(v,1)+' kWh'},{key:'usageCharges',label:'Usage charges',color:'#34d399',format:money},{key:'effective_price_per_kwh',label:'Actual $/kWh',color:'#f472b6',format:rate,axis:'right',dash:[5,5]}],{title:'Bill trend',rightAxis:true,yDigits:1,rightDigits:3,zero:false});renderDemandHeatmap($('demandHeatmap'),(DASHBOARD_DATA.demand_event_series||[]).slice(0,48));lineChart($('demandEventChart'),(DASHBOARD_DATA.demand_event_series||[]).slice(0,30).slice().reverse(),[{key:'kw',label:'Demand',color:'#22d3ee',format:v=>fmt(v,3)+' kW'},{key:'mean_temperature',label:'Mean temp',color:'#f472b6',format:v=>fmt(v,1)+'°F',axis:'right',dash:[5,5]}],{title:'Peak demand events',rightAxis:true,yDigits:3,rightDigits:1})}
renderQhControls();document.querySelectorAll('[data-tip-id]').forEach(el=>{el.onmousemove=e=>showRunTip(DASHBOARD_DATA.runs[+el.dataset.tipId.split('-')[1]],e);el.onmouseleave=hideTip});drawAll();window.onresize=drawAll;$('footer').textContent=`Static file: refresh after each scraper run · Source data: ${DASHBOARD_DATA.data_dir}`}
render();
</script></body></html>'''
